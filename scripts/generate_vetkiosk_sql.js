...
const CSV_PATH = path.join(__dirname, '../src/BD/registro horas vetkiosk - Hoja 1.csv');
const OUTPUT_DIR = path.join(__dirname, '../src/BD/vetkiosk_batches');
const VETKIOSK_CLIENT_ID = '7fb22b64-307b-439e-a845-4847838b1240';
...
async function generate() {
    ...
    for (let i = startIndex; i < lines.length; i++) {
        const [fecha, nombre, tarea, subtarea, horas] = parseCSVLine(lines[i]);
        ...
        const cleanTarea = tarea.replace(/'/g, "''");
        const cleanSubtarea = subtarea.replace(/'/g, "''");

        const taskQuery = subtarea
            ? `(SELECT t.id FROM tasks t JOIN tasks p ON t.parent_task_id = p.id WHERE t.title = '${cleanSubtarea}' AND p.title = '${cleanTarea}' AND t.client_id = '${VETKIOSK_CLIENT_ID}' LIMIT 1)`
            : `(SELECT id FROM tasks WHERE title = '${cleanTarea}' AND client_id = '${VETKIOSK_CLIENT_ID}' LIMIT 1)`;
    ...
    for (let i = 0; i < inserts.length; i += BATCH_SIZE) {
        ...
        const fileName = `vetkiosk_batch_${Math.floor(i / BATCH_SIZE)}.sql`;
